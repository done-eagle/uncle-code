...
namespace UncleCode.Analyzer
{
    ...
    public class UsingDirectiveCodeFixProvider : CodeFixProvider
    {
        ...
        private static Task<Document> SortUsingsAsync(Document document, SyntaxNode root, CancellationToken cancellationToken)
        {
            ...
            var sortedUsings = compilationUnit.Usings.OrderBy(GetUsingDirectivePriority).ToList();

            var newRoot = compilationUnit.WithUsings(SyntaxFactory.List(sortedUsings));
            return Task.FromResult(document.WithSyntaxRoot(newRoot));
        }

        private static int GetUsingDirectivePriority(UsingDirectiveSyntax usingDirective)
        {
